...
from settings import get_batfish_settings
...
def get_session(host: str | None = None):
    global _session
    host = host or get_batfish_settings().host
    if _session is None:
        try:
            from pybatfish.client.session import Session
        except ImportError as exc:
            raise RuntimeError(
                "pybatfish is not installed. Install requirements.txt before using validation."
            ) from exc
