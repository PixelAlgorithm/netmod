...
- Check that the draft does not ignore obvious current-state context from the network memory.
...
def config_agent(state: dict) -> dict:
    ...
    current_hostname = devices[0].get("hostname") if devices else None
    ...
    draft_config = template.render(**render_context)
    ...
    response = invoke_model(verify_messages)
